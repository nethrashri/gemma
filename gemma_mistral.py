...
def load_models():
    mistral_llm = Ollama(model="mistral:7b-instruct-q6_K")
    gemma_llm = Ollama(model="gemma:7b")
    return mistral_llm, gemma_llm

mistral_llm, gemma_llm = load_models()
...
# Function to generate a general query prompt
def generate_general_query_prompt(query):
    prompt = f"""
...
Please respond appropriately to the user's request or query.
"""
    return prompt
...
def handle_general_query(query):
    prompt = generate_general_query_prompt(query)
    ...
    if query_type == "Routine":
        ...
        if general_query:
            with st.spinner('Processing query...'):
                general_response = handle_general_query(general_query)
